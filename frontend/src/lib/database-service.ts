...
export class DatabaseService {
  ...
  // IPNS Record Operations
  async createIPNSRecord(
    name: string,
    currentIpfsHash: string,
    recordType: string = 'hackathons'
  ): Promise<IPNSRecord> {
    const [record] = await this.db
      .insert(ipnsRecordsTable)
      .values({
        name,
        currentIpfsHash,
        recordType,
  ...
  async getIPNSRecord(name: string): Promise<IPNSRecord | null> {
    const [record] = await this.db
      .select()
      .from(ipnsRecordsTable)
      .where(and(
        eq(ipnsRecordsTable.name, name),
        eq(ipnsRecordsTable.isActive, true)
      ))
      .limit(1);
  ...
  async updateIPNSRecord(
    name: string,
    currentIpfsHash: string,
    previousIpfsHash?: string
  ): Promise<void> {
    await this.db
      .update(ipnsRecordsTable)
      .set({
        currentIpfsHash,
        previousIpfsHash,
        updatedAt: new Date(),
      ...
      .where(eq(ipnsRecordsTable.name, name));
  }
